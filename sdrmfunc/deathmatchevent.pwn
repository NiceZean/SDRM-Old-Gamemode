...
new pDeathmatchInfo[MAX_PLAYERS][PlayerData];
...
/* samp callbacks */
forward OnFilterScriptInitTDM();
public OnFilterScriptInitTDM()
{
    CreateEventMap();
...
forward OnPlayerConnectTDM(playerid);
public OnPlayerConnectTDM(playerid)
{
    RemoveBuildingForEvent(playerid);
    return 1;
}

forward OnPlayerDisconnectTDM(playerid, reason);
public OnPlayerDisconnectTDM(playerid, reason)
{
...
forward OnPlayerDeathTDM(playerid, killerid, reason);
public OnPlayerDeathTDM(playerid, killerid, reason)
{
    if(IsInDMEvent[playerid])
    {
...
SavePlayerData(playerid)
{
    GetPlayerPos(playerid,pDeathmatchInfo[playerid][playerPos][0],pDeathmatchInfo[playerid][playerPos][1],pDeathmatchInfo[playerid][playerPos][2]);
    GetPlayerHealth(playerid, pDeathmatchInfo[playerid][playerHealth]);
    GetPlayerArmour(playerid, pDeathmatchInfo[playerid][playerArmour]);
    for (new i = 0; i < 13; i++)
    {
        GetPlayerWeaponData(playerid, i,  pDeathmatchInfo[playerid][playerWeapons][i], pDeathmatchInfo[playerid][playerAmmo][i]);
    }
    return 1;
}
...
{
    SetPlayerPos(playerid, pDeathmatchInfo[playerid][playerPos][0],pDeathmatchInfo[playerid][playerPos][1],pDeathmatchInfo[playerid][playerPos][2]);
    SetPlayerHealth(playerid, pDeathmatchInfo[playerid][playerHealth]);
    SetPlayerArmour(playerid, pDeathmatchInfo[playerid][playerArmour]);
    for (new i = 0; i < 13; i++)
    {
		GivePlayerWeapon(playerid, pDeathmatchInfo[playerid][playerWeapons][i], pDeathmatchInfo[playerid][playerAmmo][i]);
        pDeathmatchInfo[playerid][playerWeapons][i]=-1;
        pDeathmatchInfo[playerid][playerAmmo][i]=-1;
    }
    return 1;
}
